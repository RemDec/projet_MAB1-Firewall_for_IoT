...
\subsection{Vulnérabilités liées aux équipements IoT}
...
Leoglu propose dans \cite{Leloglu2017} un travail de synthèse sur base de plusieurs sources dont il résulte entre autres un modèle structural de l'IoT. Il n'est pas à considérer du point de vue d'un seul équipement, mais plutôt au niveau global ie. tout ce qui est impliqué dans le fonctionnement d'un système impliquant des équipements de l'IoT (présentant une architecture typique décrite dans la \wordlink{arch_IoT}{section}). C'est une représentation en couches, une couche supérieure s'appuyant sur les couches plus basses afin de remplir la fonction qui lui est dédiée :

\begin{figure}[!h]
\centering
\includegraphics[scale=0.65]{layers_IoT.png}
\caption{Architecture d'un système IoT proposée par Leoglu \cite{Leloglu2017}}
\label{sm_class}
\end{figure}


\subsection{ et menaces courantes}
